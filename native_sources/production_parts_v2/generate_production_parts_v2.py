...
def run(repo):
    specs = PROJECTS[repo.name]
    out = repo / "native_sources" / "production_parts_v2"
    ...
    with (out/"BOM_production_parts_v2.csv").open("w",newline="",encoding="utf-8-sig") as f:
    ...
    (out/"README.md").write_text(
        "# Production Parts V2\n\nSecond controlled batch of four parametric key parts. Each part includes validated single-solid STEP, editable millimetre DXF, A3 PDF and BOM entry. Purchased-component interfaces remain subject to final supplier drawings.\n",
        encoding="utf-8",
    )
